...
- **可选的大模型整理**：去掉口头禅和重复，理顺句间逻辑；确有并列事项或步骤时可整理为 Markdown 列表。个人词典可固定专有名词和别名。语气词和表示犹豫的省略号交给整理模型删除（`llm.system_prompt` 可替换默认 prompt）；整理结果大量删字、丢失英文、改动数字（编号列表的序号不算）或丢失词典标准词时，原识别文本会作为回退，日志写明触发的检查项。
